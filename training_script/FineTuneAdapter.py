...
import Whisper_Reprogramming.whisper as whisper
...
parser.add_argument('--project_name',help = 'vanilla_whisper or reprogramming',default = 'adapter&noise')
...
parser.add_argument('--learning_rate',help = 'calculated on fly as ',default=1e-2,type=float)
...
class ArabicDialectDataset(torch.utils.data.Dataset):
    def __init__(self, tokenizer, sample_rate = 16000, split='train',fraction=[500]) -> None:
        ...
        self.split = split
        self.dialects_to_labels = {'ALG': 0, 'EGY': 1, 'IRA': 2, 'JOR': 3, 'KSA': 4, 'KUW': 5, 'LEB': 6, 'LIB': 7, 'MAU': 8, 'MOR': 9, 'OMA': 10, 'PAL': 11, 'QAT': 12, 'SUD': 13, 'SYR': 14, 'UAE': 15, 'YEM': 16}
    ...
    def __getitem__(self, id):
        ...
        labels = self.dialects_to_labels[dialect] 
...
if __name__=="__main__":
    ...
    class MyWhisperModule(LightningModule):
        ...
        def __init__(self,cfg:Config, model_name="base") -> None:# added batch_size and lr to use auto_scale_batch_size and auto tune lr
            ...
            self.model = whisper.load_model('base',adapter=True)
        ...
        def training_step(self, batch, batch_id):
            ...
            out = self.model.decoder(dec_input_ids, audio_features,map_labels=True)
            loss = self.loss_fn(out.view(-1, out.size(-1)), labels )
